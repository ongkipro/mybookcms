...
| REQ-229 | An Owner or Admin shall be able to open one read-only `Log sistem` panel from the admin shell header and from the settings hub, at one URL-addressable admin route, listing the newest system events the runtime already persists — migration status, scheduled CAPI outbox outcomes, DOKU attempt and reconciliation events, operator notifications, and headless API audit events — in reverse chronological order with a stable label, severity, time, safe correlation id, and a link to the owning admin surface. The panel is `no-store`, bounded in count and age, never mutates state, and exposes no secret, token, password hash, click id, customer contact, address, request body, or provider payload. Customer Service and Advertiser roles have no access. | Accepted; planned |
...
| REQ-231 | Repository documentation that claims verification against disk shall describe only routes, endpoints, tables, bindings, migrations, schedules, and role grants that exist in `src/` or `wrangler.jsonc` at the revision it names, and `docs/CODE-MAP.md` shall be held to that by an executable check. A contradiction between such a document and the code is a documentation defect, fixed at the document. | Accepted; planned |
