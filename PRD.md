...
| REQ-212 | The notice shall be reachable at the point the buyer is first asked for personal data, not only from the footer, per PDPA 2010 s.7(2)(a). | Implemented locally — owner accepted 2026-09-07; A-248 |
...
| REQ-232 | When the DOKU channel a buyer already committed to is no longer enabled for the install, the buyer shall be given a stated way forward rather than a repeating failure. Retry shall continue to reuse the committed channel and shall never silently substitute another, because the channel is part of the payment intent; but the recovery surface shall name what happened and what the buyer can do, and the order shall remain inspectable and cancellable rather than stranded. | Proposal — raised by the A-242 independent review 2026-09-07 from `retryDokuPayment`, which answers `DOKU_UNAVAILABLE` 503 for as long as the operator leaves that channel disabled while the recovery page offers only "try again". Not accepted; whether the answer is copy, an operator-visible alert, an explicit cancel path, or a deliberate decision to leave it is the owner's call, and A-245 is blocked until this row is accepted |
