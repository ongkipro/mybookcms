...
| --- | --- | --- |
| REQ-173 | Store and calculate monetary values as MYR integer sen; format public values with Malaysia conventions. | Verified locally — `storefront-locale.test.ts`: "MYR display converts integer sen through Intl" |
...
| REQ-178 | Invalid/unmapped postcode, missing band, or active overlap refuses checkout before order persistence. | Verified locally — `malaysia-shipping.test.ts`: "refuses malformed, unmapped, unpriced and overlapping shipping policy" |
| REQ-179 | Authorized operators can maintain postcode zones and MYR weight-rate bands with validation against active gaps and overlaps. | Verified locally — `expedition-settings.test.ts`: "postcode range mutations reject malformed or overlapping active policy" |
...
| REQ-183 | When a buyer searches a Malaysian city, state, or postcode, checkout shall offer a selectable local directory result, populate the destination fields, and calculate the D1-owned shipping quote without an external runtime dependency. | Verified locally — `malaysia-locations.test.ts`: "location search returns only D1-backed shippable city, state, and postcode rows" |
| REQ-184 | Authorized operators can maintain an individual MYR weight-band rate for every Malaysian state and Federal Territory, while a broad zone rate remains an explicit fallback. | Verified locally — `malaysia-shipping.test.ts`: "prefers a matching state rate and keeps the broad zone rate as fallback" |
| REQ-185 | Public surfaces use one `ms-MY` document language with a controlled Malaysia-market hybrid voice: Malay owns actions, transactions, trust, address, help, legal, errors, and accessibility; familiar retail/digital terms and merchant product names may remain English. No public or admin language selector, locale cookie, translation fallback, or locale query is active. | Verified locally — `storefront-locale.test.ts`: "the Malaysia storefront always resolves to its single public locale" |
...
| REQ-202 | The public privacy notice shall describe the advertising and measurement processing the storefront actually performs, including which identity fields are hashed before transmission and which are not. No legal page may deny processing that another legal page or the running storefront discloses. | Verified locally — `malaysia-market.test.ts`: "the privacy notice does not deny processing the storefront performs" |
| REQ-203 | Public route slugs shall be Malay. A retired slug shall answer exactly one permanent redirect to its Malay replacement, preserving the query string, and only the Malay form shall appear as canonical, in the sitemap, and in internal links. `/api/*` paths and `/admin/*` routes are contracts rather than public copy and are explicitly out of scope: the operator admin stays Indonesian with English technical labels. | Verified locally — `malaysia-market.test.ts`: "a retired slug redirects once to its Malay canonical" |
| REQ-204 | A cart the active rate bands cannot carry shall be refused with the buyer's own weight and the real ceiling, both read from the active merchant rules rather than restated as a constant. | Verified locally — `malaysia-market.test.ts`: "a cart over the ceiling is told the limit, not just refused" |
...
| REQ-206 | The seller bank list shall cover the retail banks a Malaysian merchant is likely to collect a manual transfer into. | Verified locally — `seller-bank-account.test.ts`: "seller bank input rejects unsupported banks and invalid identities" |
| REQ-207 | A buyer-facing surface shall not mix Indonesian into Malay copy. One rendered component carrying both languages is a defect regardless of whether each word is individually understood. | Verified locally — `malaysia-market.test.ts`: "buyer-facing components do not mix Indonesian into Malay copy" |
| REQ-208 | A legal page the product cites in its own copy shall be reachable from the storefront, not only from `sitemap.xml`. | Verified locally — `malaysia-market.test.ts`: "the cookie policy the privacy page cites is reachable from the footer" |
| REQ-211 | The public privacy notice shall be published in Bahasa Malaysia **and** English on one page, Malay first, with no language selector. PDPA 2010 s.7(3) requires the notice in the national and English languages; `REQ-185` bars a selector. Stacking both on one document satisfies each. Scope is the notice only — the rest of the storefront stays `ms-MY`. | Implemented locally — owner accepted 2026-09-07; A-248 |
| REQ-212 | The four bilingual inline privacy paragraphs shall be omitted from the checkout recipient fields, as explicitly requested by the owner. The separate privacy page remains available. | Revised by owner 2026-09-08: “hapus aja”; A-251C supersedes the earlier inline notice placement |
| REQ-210 | The repository shall contain no host or network address belonging to one developer's machine. Seeds, fixtures and docs use documentation addresses. | Verified locally — `repository-hygiene.test.ts`: "no tracked file carries a host address outside the documentation ranges". A-267 wrote the guard and its first run found four real violations: the operator's Tailscale origin twice in `TASKS.md`, and two public fixture addresses in `rate-limit.test.ts`. All four are redacted or moved to RFC 5737 ranges |
| REQ-209 | An authorized operator can record the merchant's own pickup address — contact name, Malaysian mobile, street address, and postcode — validated by the same rules the buyer's address uses. It is stored complete or not at all, and its city and state are resolved from the postcode directory rather than stored, so they cannot disagree with the postcode. This is operator reference data: nothing is transmitted to any logistics provider. | Verified locally — `store-pickup.test.ts`: "a complete pickup address normalizes the way a buyer address does" |
...
| REQ-222 | DOKU return, result, and cancel routes shall preserve the checkout-issued order capability, expose no customer or credential data, and resolve display state from the local payment record plus a strictly validated and correlated server-to-server status retrieval under REQ-219/REQ-227 when reconciliation is needed. A callback capability in the query string shall be validated server-side, exchanged for a bounded Secure HttpOnly SameSite cookie, and removed by redirect before any page shell, advertising tag, analytics code, referrer, DOM, or browser storage can observe it. The capability shall expire server-side 24 hours after the associated payment attempt is created, regardless of cookie renewal; expiry shall refuse status and retry access without calling DOKU. A buyer shall be able to retry an eligible failed/expired initiation inside that window without creating a second order or losing attribution; if the prior terminal attempt released stock, retry shall atomically revalidate and reserve the same order items or refuse when stock is no longer available. | Verified locally 2026-09-07; response profile amended by REQ-227; sandbox lifecycle remains A-221 |
| REQ-223 | The canonical full checkout shall present COD, manual transfer, and each DOKU channel enabled for that install as a direct first-class payment choice, without a generic DOKU/all-channel choice or locally collected card fields. The selected DOKU channel shall be validated against the server-owned install policy, persisted and bound to the checkout intent, sent as the only `payment_channels` entry on initial creation, and reused unchanged by an eligible retry. The form shall explain the hosted redirect before leaving the store; preserve accessible loading, failure, cancel, pending, and recovery states; and render one consistent Malay checkout/confirmation hierarchy at 390 px and 1280 px. This work requires the repository's designer/vision handoff and depends on REQ-214. | Base checkout verified locally 2026-09-01; channel-first refinement pending A-242; sandbox remains A-221 |
| REQ-224 | The operator shall be able to inspect redacted DOKU configuration health and payment attempts, reconcile a pending attempt, and distinguish configuration, authentication, signature, timeout, provider, and local-transition failures without seeing secrets or raw customer payloads. Scheduled reconciliation shall be bounded, idempotent, and limited to eligible non-terminal attempts. | Partially verified. The attempt, authentication, signature, timeout, provider, and local-transition halves were verified locally 2026-09-01 and stand. The configuration half did not: until 2026-09-07 a configured record whose credential could not be decrypted was swallowed silently, so an operator could not distinguish it from an install that never configured DOKU — on the buyer surface or in the logs. A-243 adds the `doku-config-unusable` signal and registers it in `OBSERVABILITY.md`; that work is implemented and test-verified but its independent review is not yet bound, so this row does not yet read verified. Corrected here rather than left standing, per REQ-231 |
...
| REQ-232 | When the committed DOKU channel is no longer enabled, recovery shall explain the restriction, stop repeated retries and offer order tracking and merchant contact. Retry never substitutes a channel. Existing order inspection and operator cancellation remain available; no automatic cancellation or channel switch is introduced. | Implemented locally — owner accepted under ADR-031; A-245 |
...
## Checkout recovery accepted 2026-09-08

| ID | Requirement | State |
| --- | --- | --- |
| REQ-233 | CRM shall expose a separate `{{variant_name}}` token while retaining existing `{{product_name}}` rendering. The full checkout shall capture a recoverable lead after valid receiver name, Malaysia mobile and active variant are entered, with recovery processing described on the privacy page; the owner removed the inline checkout disclosure on 2026-09-08. Leads shall not reserve stock, create payment attempts, emit Purchase or enter order revenue/counts. Owner/Admin/CS can list/search leads, explicitly record follow-up status and notes, and convert a lead into one COD order after entering a valid address and selecting a trusted Malaysia location and current quote. COD availability, catalog price and stock remain server-authoritative. Buyer completion and concurrent CS conversion must link a lead to at most one order atomically; converted leads leave the pending queue. Public capture must not expose personal data or permit lookup by phone. | Accepted by owner: “lanjut sempurnakan”; implemented and verified locally 2026-09-08 |

## Complete landing builder accepted 2026-09-08

| ID | Requirement | State |
| --- | --- | --- |
| REQ-234 | Owner/Admin/Advertiser shall create and edit complete CMS landing pages with title, custom slug, product binding, draft/publication and SEO; ordered headline, paragraph, numbered-list, bullet-list, image, HTML and Malaysia full-checkout sections; text alignment/headline size, bounded image upload/alt, reorder/duplicate/remove/jump controls, safe local preview and saved-page preview. Structured content must round-trip through D1 and public rendering while preserving existing HTML/form pages. Product changes must not retain another product's default variant. Failed loading/saving/uploading must preserve work and offer recovery; unsaved navigation must warn and manual slugs must survive title changes. Checkout titles/button copy must render when configured, with payment-specific DOKU labels retaining their meaning. Existing roles, stock, payment, tracking and native-page ownership remain authoritative; no new builder dependency is required. | Accepted by owner: “landingpage builder lengkap”, like AdsBookCMS |
...
REQ-234 presentation amendment accepted 2026-09-08: image-only sections fill the public column, preserve aspect ratio, and have no padding, margin, or rounded corners so adjacent images join seamlessly. Text-section spacing remains unchanged.

## Progressive checkout accepted 2026-09-08

| ID | Requirement | State |
| --- | --- | --- |
| REQ-235 | Shared checkout shall use clear product/contact/delivery/payment/summary sections in one responsive page. Payment controls stay hidden and disabled until a directory-selected postcode/city has a successful current shipping quote, and are hidden again while location/variant changes invalidate that quote. Loading, errors and retry remain visible; hidden payment/email controls cannot receive focus or block validation. Previous method/email values survive re-selection. Payment methods load only once the gate opens; DOKU channels/disclosures, provider/stock/order safeguards and uncertain-submit retry behavior remain intact. Unknown shipping shall not be presented as free or included in a final total. | Accepted: owner asks payment after postcode/city and Shopify-like checkout UI/UX; A-252 |


REQ-193 advertising-currency amendment accepted 2026-09-08: Meta Pixel/CAPI,
direct Google Ads conversions and GTM ecommerce event values use IDR at a fixed
4,100 IDR per MYR. Commerce event producers and the Meta ingress contract retain
MYR major-unit inputs; conversion happens once at each outbound channel boundary.
Orders, payment/checkout amounts and Google/Meta catalog XML remain MYR.
Purchase remains merchandise-only with existing timing and identifiers. Persisted
outbox payloads retain their original amounts/currency on retry; do not rewrite
or replay historical events. This rule does not prove receipt, attribution or ROAS.
