...
- [ ] **A-224** — Restructure the Malaysia shipping-policy workspace around operator jobs.
...
- [ ] **A-225** — Add a `Log sistem` action to the admin shell that opens one read-only, redacted system-event panel.
...
- [ ] **A-227** — Make `docs/CODE-MAP.md` fail a test when it drifts from `src/`.
...
- [ ] **A-228** — Re-verify every `Verified against disk` document at the current HEAD and close the ledger gap.
