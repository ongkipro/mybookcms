...
- [x] **A-256** — Make the admin canvas obey the design system it already has. **Done 2026-09-09.** 660 raw palette uses in `src/components/admin` converted to the tokens the semantic layer already named: `text-slate-500`→`text-muted-foreground` (187), `text-slate-950`/`900`→`text-foreground` (167), `border-slate-200`→`border-border` (151), `bg-slate-50`+`bg-slate-100`→`bg-muted` (162, decision (c): one recessed ground), `bg-white`→`bg-card` (90), `text-slate-700`→ the new `--foreground-subtle` (73, decision (b)). `zinc` is gone.
      **The sweep improves contrast rather than only tidying vocabulary**, verified by computing the ratios rather than trusting the handoff: `--muted-foreground` resolves to `#5f6a77` and measures 5.50 on a white card, 5.13 on the page, 4.97 on a muted panel — all pass AA — while the `text-slate-500` it replaced measures 4.76 / **4.44** / **4.31**, failing on two of the three grounds it was used against. 187 potential failures closed.
      Step 0 first, as the entry required: `admin.css` matched card geometry on the literal class `bg-white`, so the sweep would have dropped radius and shadow on sixteen sections. Both `.bg-white` and `.bg-card` are listed now; `.bg-white` goes when no component uses it.
      **A third stale premise, after the two the entry already corrected.** It said `text-slate-600` stays raw "in exactly seventeen occurrences across three files". It is 90 across 18. The rule is what matters, not the count: `DESIGN-SYSTEM.md` records that muted text on the page background uses slate-600 for 7.26:1 because slate-500 there is 4.41. Mapping them to `--muted-foreground` would pass AA and still *reduce* contrast on text that was deliberately darkened, so all 90 stay. `text-slate-400` also stays — the handoff split it by role, which needs reading each occurrence.
      `src/lib/admin-token-guard.test.ts` is the mechanism that notices next time: it forbids the eight converted shades by name, keeps zinc retired, and asserts the three deliberate carve-outs are still *present* so a future sweep cannot quietly convert them. Mutation-proved by reintroducing `text-slate-500` and `bg-white` into `OrdersTable.tsx`.
      Six admin routes verified in a real browser at 390 px and 1280 px — no overflow, no console errors, no runtime exceptions — with two assertions specific to this change: every card still computes a non-zero border radius, and `text-foreground-subtle` resolves to a colour rather than to nothing.
      `src/layouts/AdminLayout.astro` is included, as decision (a) required: line 38 carried `text-slate-900` on `<body>`, the ink every component inherits. I nearly closed this claiming it did not, and checking before writing the claim is the only reason that is not in the repository now. The guard covers the layout for the same reason.
- [x] **A-276** — Extend the admin's one vocabulary to the route files A-256 left out. **Done 2026-09-09.** 253 conversions across `src/pages/admin`, the same eight mappings A-256 established. The entry said ten routes; there are **25 `.astro` files**, one more stale figure inherited from the original census. The A-256 guard now walks that directory recursively, so it covers 25 route files, 25 components and the layout — and it is the only automated check those routes will ever have, since ADR-032 records that Biome reports 81 false errors on `.astro` frontmatter. Mutation-proved against a route by reintroducing `text-slate-500` into `settings.astro`. All 22 reachable admin routes verified in a real browser at 390 px and 1280 px: no overflow, no console errors, no runtime exceptions, card radius intact, and all three inks resolving. The same carve-outs stand — `text-slate-600` (27 here), `text-slate-400`, and the dark surfaces.
