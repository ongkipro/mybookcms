...
- [x] **A-271** — Give the schema log entry somewhere to go, and the operator something to read when it goes red. **Done 2026-09-09.** `/admin/settings/schema` states the expected version, the applied version, the state, the error code, and the next action — and says outright that it cannot repair anything, because the admin deliberately has no migration control. `system-log.ts` no longer returns `href: null` for the one source that can report `severity: "error"`; it links whether or not the state is healthy, since the version is worth reading before something breaks.
