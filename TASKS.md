...
- [ ] **A-234** — Close the two places the admin misses its own accessibility bar. Both predate this session and both are system-wide.
      ...
      *Text contrast.* A Lighthouse pass on `/admin/settings/log` scored accessibility 96 with one failure and best practices 100. The failure is real and narrower than it first looks: `text-slate-500` is `#62748e`, which gives **4.76** on a white card and passes, but **4.41** on the admin page background `#f5f6f8` and fails the 4.5 AA floor. So the 277 occurrences across admin code are mostly fine — only text sitting directly on the page background is not. Lighthouse found exactly two such nodes, the eyebrow and the description in `AdminPageHeader.astro`, which 20 admin pages render. `slate-600` on the same background gives 7.01. The back link in that component uses the same class and needs the same look.
