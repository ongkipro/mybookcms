...
| `/admin/settings/schema` | [system-log.test.ts](../src/lib/system-log.test.ts) — Direct runtime: the `schema` log entry's destination is pinned and the guidance for all six states is asserted. [auth.test.ts](../src/lib/auth.test.ts) — Direct runtime: owner and admin reach it, Customer Service and Advertiser are refused, and the answer is asserted equal to `/admin/settings/log`'s. | [STATUS A-271](../STATUS.md#a-271--the-schema-entry-has-somewhere-to-go-2026-09-09): real browser at 390/1280 px, all four required facts present, and the log verified to link here. | Read-only. The admin exposes no migration control and A-271 added none. |
