...
export function AbandonedOrders() {
  ...
  return <section className="space-y-4">
    ...
    <div className="grid grid-cols-1 gap-3 lg:hidden">{rows.map(lead => <article key={lead.id} className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
    ...
    {!!rows.length && <div className="hidden overflow-x-auto rounded-xl border border-slate-200 bg-white lg:block"><table className="w-full table-fixed text-left text-sm"><caption className="sr-only">Checkout yang belum selesai dan tindak lanjut customer service</caption><thead className="bg-slate-50 text-xs text-slate-600"><tr>{['Customer', 'Produk & varian', 'Status', 'Aksi'].map(h => <th key={h} scope="col" className="p-4">{h}</th>)}</tr></thead><tbody>{rows.map(lead => <tr key={lead.id} className="border-t border-slate-200 align-top">
      ...
      <td className="break-words p-4">{lead.product_name}<p className="text-slate-500">{lead.variant_name} · 1 item</p></td>
    ...
    <form className="space-y-4" onSubmit={async e => {
      ...
      <fieldset disabled={pending} className="space-y-4">
        {mode === 'follow' ? <>
          ...
          <dl className="space-y-2 rounded-lg bg-slate-50 p-3 text-sm"><div className="flex justify-between"><dt>Produk</dt><dd>{variant ? formatMyr(variant.price) : '—'}</dd></div><div className="flex justify-between"><dt>Pengiriman</dt><dd>{amount !== null ? formatMyr(amount) : quoteError ? 'Tarif belum tersedia' : location ? 'Memuat tarif…' : 'Pilih lokasi'}</dd></div><div className="flex justify-between font-bold"><dt>Total COD</dt><dd>{variant && amount !== null ? formatMyr(variant.price + amount) : '—'}</dd></div></dl>
