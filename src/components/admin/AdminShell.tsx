...
export function AdminShell({
  ...
  const searchContent = (
    <>
      <div className="border-b border-border bg-muted/70 p-2">
        <div className="rounded-lg border border-slate-300 bg-card shadow-sm focus-within:border-slate-400 focus-within:ring-2 focus-within:ring-slate-200">
          <CommandInput
            value={searchQuery}
      ...
      <CommandList className="max-h-none flex-1 overflow-y-auto px-2 py-2 sm:max-h-[min(55vh,28rem)]">
        <CommandEmpty className="px-4 py-14 text-center">
          <p className="font-bold text-foreground">Tidak ada halaman yang cocok</p>
      ...
      <div className="hidden items-center justify-between border-t border-border bg-muted px-4 py-2.5 text-[10px] font-semibold text-muted-foreground sm:flex">
    ...
    <TooltipProvider>
      <SidebarProvider open={sidebarOpen}>
        ...
        <SidebarInset className="min-w-0 bg-transparent">
          <header className="admin-topbar sticky top-0 z-30 flex h-[60px] shrink-0 items-center border-b border-border/80 bg-card/95 px-3 backdrop-blur-xl md:h-[68px] md:px-6">
            <div className="flex min-w-0 flex-1 items-center gap-2.5 md:gap-3">
              ...
              <div className="min-w-0">
                <h1 className="truncate text-[15px] font-extrabold tracking-[-0.02em] text-foreground md:text-base">
                  <span className="md:hidden">{title}</span>
                  <span className="hidden md:inline">{title}</span>
                </h1>
                <p className="hidden truncate text-[11px] font-medium text-muted-foreground lg:block">
            ...
            {!mustChangePassword && <button
              ...
              className="group mx-4 hidden min-h-10 w-full max-w-lg flex-1 items-center gap-2.5 rounded-xl border border-border bg-muted/80 px-3 text-left text-sm font-medium text-muted-foreground transition hover:border-slate-300 hover:bg-card hover:text-foreground-subtle focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 md:flex xl:mx-6"
            >
              <Search className="size-4 text-slate-400" aria-hidden="true" />
              <span className="min-w-0 flex-1 truncate">Cari halaman atau pengaturan</span>
              <kbd className="rounded-md border border-border bg-card px-1.5 py-0.5 font-mono text-[10px] font-semibold text-slate-600">⌘K</kbd>
            </button>}
            ...
            <div className="flex flex-1 items-center justify-end gap-1.5 md:gap-2">
              {!mustChangePassword && <button
                type="button"
                onClick={() => setSearchOpen(true)}
                className="grid size-10 place-items-center rounded-xl text-slate-600 transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 md:hidden"
                aria-label="Cari halaman admin"
              >
                <Search className="size-[18px]" aria-hidden="true" />
              </button>}
              {!mustChangePassword && <a href="/" target="_blank" rel="noopener noreferrer" className="hidden min-h-10 items-center gap-1.5 rounded-xl px-3 text-xs font-bold text-slate-600 transition hover:bg-muted hover:text-foreground xl:inline-flex">
              ...
              {!mustChangePassword && (adminRole === "owner" || adminRole === "admin") && (
                <a
                  ...
                  className="grid size-10 place-items-center rounded-xl text-slate-600 transition hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300"
          ...
          <nav aria-label="Navigasi utama mobile" className="admin-bottom-nav fixed inset-x-0 bottom-0 z-40 border-t border-border/80 bg-card/95 shadow-[0_-8px_30px_rgba(15,23,42,0.06)] backdrop-blur-xl md:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
            <div className="grid h-16 items-stretch" style={{ gridTemplateColumns: `repeat(${mobilePrimary.length + 1}, minmax(0, 1fr))` }}>
              {mobilePrimary.map((item) => {
                const active = activeMenu === item.id;
                return (
                  <a key={item.href} href={item.href} aria-current={isAdminNavHrefActive(currentPath, item.href) ? "page" : undefined} className={`group flex min-w-0 flex-col items-center justify-center gap-1 px-1 text-[10px] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-slate-300 ${active ? "font-medium text-foreground" : "font-normal text-muted-foreground"}`}>
              ...
              <button id="admin-mobile-menu-trigger" type="button" onClick={() => setMenuOpen(true)} aria-current={menuIsActive ? "page" : undefined} className={`flex min-w-0 flex-col items-center justify-center gap-1 px-1 text-[10px] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-slate-300 ${menuIsActive ? "font-medium text-foreground" : "font-normal text-muted-foreground"}`} aria-label={mustChangePassword ? "Buka menu akun" : "Buka semua menu"}>
        ...
        <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
          <SheetContent
            side="bottom"
            className="max-h-[86dvh] gap-0 overflow-hidden rounded-t-[1.25rem] border-border bg-card p-0 md:hidden"
          ...
          >
            <SheetHeader className="border-b border-border px-4 pb-3 pt-4 text-left">
              <SheetTitle className="text-base font-semibold tracking-tight text-foreground">{mustChangePassword ? "Keamanan akun" : "Semua menu"}</SheetTitle>
            ...
            <div className="overflow-y-auto overscroll-contain px-2 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-2">
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        <div key={item.id} className="border-b border-slate-100 last:border-b-0">
                          <div className="flex min-h-13 items-center gap-2 px-3 py-2 active:bg-muted">
                            ...
                            <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-muted text-slate-600" style={active ? { backgroundColor: `${ADMIN_ACCENT}14`, color: ADMIN_ACCENT } : undefined}>
                              <item.icon className="size-4" aria-hidden="true" />
                            </span>
                            <span className="min-w-0 flex-1">
                              <span className={`block truncate text-sm text-foreground ${active ? "font-medium" : "font-normal"}`}>{item.label}</span>
                              <span className="block truncate text-[11px] text-muted-foreground">{item.description}</span>
                            </span>
                            {!hasChildren && (
                              <ChevronRight className="size-4 shrink-0 text-slate-300" aria-hidden="true" />
                            ...
                            {hasChildren && (
                              <button
                                type="button"
                                className="grid size-9 shrink-0 place-items-center rounded-lg text-muted-foreground hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
                          ...
                          {item.children && expanded && (
                            <div id={`mobile-submenu-${item.id}`} className="bg-muted/50 pb-1.5 pl-14 pr-3 pt-0">
                              {item.children.map((child) => {
                                ...
                                return (
                                  <a key={child.href} href={child.href} aria-current={childActive ? "page" : undefined} className={`flex min-h-11 items-center justify-between gap-3 border-t border-slate-100/60 py-1.5 text-sm active:bg-muted/50 ${childActive ? "font-medium text-foreground" : "font-normal text-slate-600"}`}>
              ...
              <a href={profileNavItem.href} className="mb-2 flex min-h-13 items-center gap-3 rounded-xl border border-border bg-card px-3 py-2">
                <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-muted text-slate-600"><profileNavItem.icon className="size-4" /></span>
                <span className="min-w-0 flex-1"><span className="block text-sm font-normal text-foreground">{profileNavItem.label}</span><span className="block truncate text-[11px] text-muted-foreground">{profileNavItem.description}</span></span>
        ...
        {!mustChangePassword && (isDesktop ? (
          ...
          >
            <div className="border-b border-border px-4 py-3 pr-12">
              <h2 className="text-sm font-extrabold tracking-tight text-foreground">Pencarian admin</h2>
          ...
          <Sheet open={searchOpen} onOpenChange={handleSearchOpenChange}>
            <SheetContent side="bottom" style={{ height: "min(82dvh, 42rem)" }} className="max-h-[calc(100dvh-env(safe-area-inset-top))] gap-0 overflow-hidden rounded-t-[1.25rem] border-border bg-card p-0">
              <SheetHeader className="border-b border-border px-4 pb-3 pt-4 pr-12 text-left">
                <SheetTitle className="text-base font-extrabold tracking-tight text-foreground">Pencarian admin</SheetTitle>
