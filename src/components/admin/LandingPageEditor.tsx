...
export default function LandingPageEditor({landingPageId}: {landingPageId?: string}) {
  ...
  function sectionPreview(section: Section) {
    if (section.type === "form") {
      ...
      return <div className="space-y-4 p-5 text-sm text-zinc-800"><p className="text-xs text-zinc-500">Preview form Malaysia · pengiriman dihitung saat checkout</p><h2 className="text-xl font-bold">{section.form_config?.section_title || "Maklumat pesanan"}</h2><p>{product?.title || "Pilih produk terlebih dahulu"}</p><p>{variant?.title} {variant ? formatMyr(variant.price) : ""}</p>{["Nama penuh", "Nombor WhatsApp", "Alamat penghantaran", "Poskod"].map(label => <div key={label} className="rounded-lg border p-3 text-zinc-500">{label}</div>)}<div className="rounded-lg bg-zinc-900 p-3 text-center text-white">{section.form_config?.button_text || "Hantar pesanan"}</div><p className="text-xs text-zinc-500">Metode pembayaran mengikuti pengaturan toko. Label pembayaran DOKU mengikuti metode yang dipilih.</p></div>;
    }
    try {
      ...
      <main className={`${mobileView === 'content' ? '' : 'hidden xl:block'} min-w-0 rounded-xl border border-border bg-muted/30 p-2 sm:p-5`} aria-label="Kanvas landing page">
        ...
        <div className="mx-auto w-full max-w-[480px] min-w-0 space-y-3">
          ...
          {draft.sections.map((section, index) => {
            ...
            return <section id={`builder-${section.id}`} key={section.id} data-section-type={section.type} className={`min-w-0 overflow-hidden rounded-xl border bg-white text-zinc-900 ${activeSection === section.id ? 'border-blue-500 ring-1 ring-blue-500/20' : 'border-border'}`}>
