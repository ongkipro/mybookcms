    ...
    {!loading && shipments.length > 0 ? <>
      ...
      <section className="grid grid-cols-1 gap-3 lg:hidden" aria-label="Daftar pengiriman mobile">
        {shipments.map((shipment) => <article key={shipment.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm" aria-label={`Pengiriman ${shipment.orderNumber}`}>
          <div className="flex items-start justify-between gap-3"><div><a className="font-black text-blue-700 hover:underline" href={`/admin/orders/${encodeURIComponent(shipment.orderNumber)}`}>{shipment.orderNumber}</a><p className="mt-1 text-[10px] text-slate-500">{formatDateTime(shipment.createdAt)}</p></div><StatusBadge status={shipment.shippingStatus} /></div>
