  ...
  return <div className="space-y-5">
    <header className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-black text-foreground">Pengiriman</h1>
    ...
    <section className="rounded-xl border border-border bg-card p-4 shadow-sm" aria-label="Filter pengiriman">
      ...
      <p className="mt-3 text-xs font-bold text-muted-foreground" aria-live="polite">{refreshing ? "Memperbarui antrean…" : `${total} pesanan dalam antrean sesuai filter${total > 200 ? " · menampilkan 200 terbaru" : ""}`}</p>
    ...
    {loading ? <div className="space-y-3" aria-label="Memuat antrean pengiriman" aria-busy="true"><div className="hidden h-80 animate-pulse rounded-xl bg-muted lg:block" />{[0, 1, 2].map((id) => <div key={id} className="h-64 animate-pulse rounded-xl bg-muted lg:hidden" />)}</div> : null}

    {!loading && !error && shipments.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-card p-10 text-center"><Truck className="mx-auto size-8 text-slate-400" /><p className="mt-3 font-black text-foreground">{filtered ? "Tidak ada pengiriman yang sesuai filter" : "Belum ada pesanan di Pengiriman"}</p><p className="mt-1 text-sm text-muted-foreground">{filtered ? "Atur ulang filter atau gunakan pencarian lain." : "Gunakan aksi “Masukkan ke Pengiriman” dari Manajemen Pesanan."}</p><div className="mt-5">{filtered ? <Button variant="outline" onClick={() => { setQuery(""); setFilter("all"); }}>Atur ulang filter</Button> : <a href="/admin/orders" className={buttonVariants({ variant: "default" })}>Buka Manajemen Pesanan</a>}</div></div> : null}

    {!loading && shipments.length > 0 ? <>
      <section className="hidden overflow-hidden rounded-xl border border-border bg-card shadow-sm lg:block" aria-label="Tabel pengiriman desktop">
        <div className="overflow-x-auto"><Table className="min-w-[920px] table-fixed">
          <TableHeader><TableRow className="bg-muted text-[11px] font-black uppercase tracking-wider text-muted-foreground"><TableHead className="w-[140px] px-4">Pesanan</TableHead><TableHead className="w-[250px] px-4">Penerima & alamat</TableHead><TableHead className="w-[155px] px-4">Pembayaran</TableHead><TableHead className="w-[145px] px-4">Status & ongkir</TableHead><TableHead className="w-[230px] px-4 text-right">Aksi</TableHead></TableRow></TableHeader>
          <TableBody>{shipments.map((shipment) => <TableRow key={shipment.id} className="align-top hover:bg-muted"><TableCell className="px-4 py-4"><a className="font-black text-foreground hover:text-blue-700 hover:underline" href={`/admin/orders/${encodeURIComponent(shipment.orderNumber)}`}>{shipment.orderNumber}</a><p className="mt-1 text-[10px] text-muted-foreground">{formatAdminDateTime(shipment.createdAt)}</p></TableCell><TableCell className="px-4 py-4"><p className="font-black text-foreground">{shipment.customerName}</p><p className="mt-1 font-mono text-[11px] text-muted-foreground">{shipment.customerPhone}</p><p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">{[shipment.address, shipment.city, shipment.province, shipment.postcode].filter(Boolean).join(", ")}</p></TableCell><TableCell className="px-4 py-4"><StatusBadge status={shipment.paymentStatus} payment /><p className="mt-2 text-xs font-bold text-foreground-subtle">{paymentMethods[shipment.paymentMethod] || shipment.paymentMethod}</p>{shipment.paymentMethod === "cod" ? <p className="mt-1 text-xs font-black text-foreground">COD {formatMyr(shipment.totalAmount)}</p> : null}</TableCell><TableCell className="px-4 py-4"><StatusBadge status={shipment.shippingStatus} /><p className="mt-2 text-xs font-black text-foreground">{formatMyr(shipment.shippingCost)}</p><p className="mt-1 text-[10px] uppercase tracking-wide text-muted-foreground">{shipment.shippingZoneCode || "Zona belum tersedia"}</p></TableCell><TableCell className="px-4 py-4"><div className="flex flex-wrap justify-end gap-1"><a href={`/admin/orders/${encodeURIComponent(shipment.orderNumber)}`} className={buttonVariants({ variant: "ghost", size: "sm" })}><Eye />Lihat</a><Button size="sm" onClick={() => openEditor(shipment)}><PackageCheck />Perbarui</Button><Button variant="outline" size="sm" onClick={() => void release(shipment)} disabled={releasingIds.includes(shipment.id)}>{releasingIds.includes(shipment.id) ? <LoaderCircle className="animate-spin" /> : <ListMinus />}{releasingIds.includes(shipment.id) ? "Mengeluarkan…" : "Keluarkan"}</Button></div></TableCell></TableRow>)}</TableBody>
        </Table></div>
      </section>

      <section className="grid grid-cols-1 gap-3 lg:hidden" aria-label="Daftar pengiriman mobile">
        {shipments.map((shipment) => <article key={shipment.id} className="rounded-xl border border-border bg-card p-4 shadow-sm" aria-label={`Pengiriman ${shipment.orderNumber}`}>
          <div className="flex items-start justify-between gap-3"><div><a className="font-black text-blue-700 hover:underline" href={`/admin/orders/${encodeURIComponent(shipment.orderNumber)}`}>{shipment.orderNumber}</a><p className="mt-1 text-[10px] text-muted-foreground">{formatAdminDateTime(shipment.createdAt)}</p></div><StatusBadge status={shipment.shippingStatus} /></div>
          <div className="mt-4 border-t border-slate-100 pt-4"><p className="font-black text-foreground">{shipment.customerName}</p><p className="mt-1 font-mono text-xs text-muted-foreground">{shipment.customerPhone}</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{[shipment.address, shipment.city, shipment.province, shipment.postcode].filter(Boolean).join(", ")}</p></div>
          <div className="mt-4 grid grid-cols-2 gap-3 rounded-xl bg-muted p-3 text-xs"><div><p className="text-muted-foreground">Pembayaran</p><p className="mt-1 font-black text-foreground">{paymentMethods[shipment.paymentMethod] || shipment.paymentMethod}</p><p className="mt-1">{paymentLabels[shipment.paymentStatus] || shipment.paymentStatus}</p></div><div><p className="text-muted-foreground">Ongkir</p><p className="mt-1 font-black text-foreground">{formatMyr(shipment.shippingCost)}</p><p className="mt-1 uppercase">{shipment.shippingZoneCode || "-"}</p></div></div>
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {mayWriteShippingCost ? (
            ...
            <div className="text-xs font-bold text-slate-600">
              Biaya pengiriman
              <p className="mt-1 text-sm font-black text-foreground">RM {draft.shippingCostMyr}</p>
